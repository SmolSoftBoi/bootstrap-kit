...
(function ($, undefined) {
  ...
  Datepicker.prototype = {
    ...
    _process_options(opts) {
      ...
      if (o.startDate !== Number.NEGATIVE_INFINITY) {
        if (o.startDate) {
          o.startDate = o.startDate instanceof Date ? this._local_to_utc(this._zero_time(o.startDate)) : DPGlobal.parseDate(o.startDate, format, o.language);
        } else {
      ...
      if (o.endDate !== Number.POSITIVE_INFINITY) {
        if (o.endDate) {
          o.endDate = o.endDate instanceof Date ? this._local_to_utc(this._zero_time(o.endDate)) : DPGlobal.parseDate(o.endDate, format, o.language);
        } else {
      ...
      o.daysOfWeekDisabled = $.map(o.daysOfWeekDisabled, d => {
          ...
          'mousedown touchstart': $.proxy(function (e) {
            ...
            if (!(
              this.element.is(e.target) ||
							this.element.find(e.target).length > 0 ||
							this.picker.is(e.target) ||
							this.picker.find(e.target).length > 0
            )) {
              this.hide();
            }
          }, this)
    ...
    place() {
      ...
      const windowWidth = $(this.o.container).width();
      const windowHeight = $(this.o.container).height();
      const scrollTop = $(this.o.container).scrollTop();
      const appendOffset = $(this.o.container).offset();

      const parentsZindex = [];
      this.element.parents().each(function () {
        const itemZIndex = $(this).css('z-index');
        if (itemZIndex !== 'auto' && itemZIndex !== 0) {
          parentsZindex.push(Number.parseInt(itemZIndex));
        }
      });
      const zIndex = Math.max.apply(Math, parentsZindex) + 10;
      ...
      let top_overflow; let bottom_overflow;
      if (yorient === 'auto') {
    ...
    update() {
      ...
      if (fromArgs) {
        ...
        this.setValue();
      } else if (dates.length > 0 && // Setting date by typing
				String(oldDates) !== String(this.dates)) {
        this._trigger('changeDate');
      }

      if (this.dates.length === 0 && oldDates.length > 0) {
    ...
    fill() {
      ...
      const startYear = this.o.startDate !== Number.NEGATIVE_INFINITY ? this.o.startDate.getUTCFullYear() : Number.NEGATIVE_INFINITY;
      const startMonth = this.o.startDate !== Number.NEGATIVE_INFINITY ? this.o.startDate.getUTCMonth() : Number.NEGATIVE_INFINITY;
      const endYear = this.o.endDate !== Number.POSITIVE_INFINITY ? this.o.endDate.getUTCFullYear() : Number.POSITIVE_INFINITY;
      const endMonth = this.o.endDate !== Number.POSITIVE_INFINITY ? this.o.endDate.getUTCMonth() : Number.POSITIVE_INFINITY;
        ...
        if (prevMonth.getUTCDay() === this.o.weekStart) {
          ...
          if (this.o.calendarWeeks) {
            ...
            const
              ...
              ws = new Date(Number(prevMonth) + (this.o.weekStart - prevMonth.getUTCDay() - 7) % 7 * 864e5);
            // Thursday of this week
            const th = new Date(Number(ws) + (7 + 4 - ws.getUTCDay()) % 7 * 864e5);
      ...
      html = '';
      year = Number.parseInt(year / 10, 10) * 10;
    ...
    updateNavArrows() {
      ...
      case 0:
        if (this.o.startDate !== Number.NEGATIVE_INFINITY && year <= this.o.startDate.getUTCFullYear() && month <= this.o.startDate.getUTCMonth()) {
          this.picker.find('.prev').css({ visibility: 'hidden' });
        } else {
          this.picker.find('.prev').css({ visibility: 'visible' });
        }

        if (this.o.endDate !== Number.POSITIVE_INFINITY && year >= this.o.endDate.getUTCFullYear() && month >= this.o.endDate.getUTCMonth()) {
      ...
      case 1:
      case 2:
        if (this.o.startDate !== Number.NEGATIVE_INFINITY && year <= this.o.startDate.getUTCFullYear()) {
          this.picker.find('.prev').css({ visibility: 'hidden' });
        } else {
          this.picker.find('.prev').css({ visibility: 'visible' });
        }

        if (this.o.endDate !== Number.POSITIVE_INFINITY && year >= this.o.endDate.getUTCFullYear()) {
          this.picker.find('.next').css({ visibility: 'hidden' });
        } else {
          this.picker.find('.next').css({ visibility: 'visible' });
        }
      ...
      if (target.length === 1) {
        ...
        case 'span':
          if (!target.hasClass('disabled')) {
            ...
            if (target.hasClass('month')) {
              ...
              day = 1;
              month = 0;
              year = Number.parseInt(target.text(), 10) || 0;
        ...
        case 'td':
          if (target.hasClass('day') && !target.hasClass('disabled')) {
            day = Number.parseInt(target.text(), 10) || 1;
            year = this.viewDate.getUTCFullYear();
    ...
    keydown(e) {
      ...
      let dir; let newDate; let newViewDate;
  ...
  const DateRangePicker = function (element, options) {
    this.element = $(element);
    this.inputs = $.map(options.inputs, i => {
    ...
    this.pickers = $.map(this.inputs, i => {
  ...
  DateRangePicker.prototype = {
    updateDates() {
      this.dates = $.map(this.pickers, i => {
        return i.getUTCDate();
      });
      this.updateRanges();
    },
    updateRanges() {
      const range = $.map(this.dates, d => {
        return d.valueOf();
      });
  ...
  var datepickerPlugin = function (option) {
    const args = Reflect.apply(Array, null, arguments);
    args.shift();
    let internal_return;
    this.each(function () {
      const $this = $(this);
  ...
  var DPGlobal = {
    ...
    parseDate(date, format, language) {
      ...
      let part; let dir; let i;
      if (/^[+\-]\d+[dmwy]([\s,]+[+\-]\d+[dmwy])*$/.test(date)) {
        date = new Date();
        for (i = 0; i < parts.length; i++) {
          part = part_re.exec(parts[i]);
          dir = Number.parseInt(part[1]);
          switch (part[2]) {
          case 'd':
      ...
      let val; let filtered;
      setters_map.M = setters_map.MM = setters_map.mm = setters_map.m;
      setters_map.dd = setters_map.d;
      date = UTCDate(date.getFullYear(), date.getMonth(), date.getDate(), 0, 0, 0);
      let fparts = [...format.parts];
      // Remove noop parts
      if (parts.length !== fparts.length) {
        fparts = $(fparts).filter((i, p) => {
